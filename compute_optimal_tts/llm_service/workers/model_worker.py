...
# It implements the policy model worker (transformers-based generation).
...
class ModelWorker(BaseModelWorker):
    def __init__(
        ...
        **kwargs,
        ...
        from compute_optimal_tts.llm_service.workers.inference import generate_stream
    ...
    def generate_gate(self, params):
        ...
        ret = {}
        ret["text"] = [stream_result.get("text", "")]
        ret["finish_reason"] = [stream_result.get("finish_reason", "unknown")]
        usage = stream_result.get("usage", {}) or {}
        ret["usage"] = {"prompt_tokens": [usage.get("prompt_tokens", 0)]}
        ret["output_token_len"] = [usage.get("completion_tokens", 0)]
        logprobs = stream_result.get("logprobs", {}) or {}
        token_logprobs = logprobs.get("token_logprobs", []) or []
        try:
            ret["cumulative_logprob"] = [float(sum(token_logprobs))]
        except Exception:
            ret["cumulative_logprob"] = [0.0]
        return ret
