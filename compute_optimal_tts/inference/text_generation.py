...
# It implements FastChat/SGLang text generation request wrappers.
...
def _generate_fastchat(
    ...
    results = None
    try:
        response = requests.post(worker_addr + "/worker_generate", headers=headers, json=gen_params)
        if not response.text:
            raise ValueError("empty response body")
        results = response.json()
    except Exception as e:
        msg = f"Error in _generate_fastchat: {e}"
        if response is not None:
            msg += f"\nstatus={getattr(response,'status_code',None)} body_head={response.text[:500]!r}"
        print(msg)
        raise
    ...
    for output in results:
        ...
        if include_stop_str_in_output and finish_reason['type'] == 'stop' and 'matched' in finish_reason.keys():
            try:
                ...
                traceback.print_exc()
                print(f'Error in _generate_sgl processing finish_reason: {e}')
